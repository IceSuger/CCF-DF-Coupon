...
#读取样本权重
sample_weight = readAsChunks_nohead("sample_weight.csv",{0:float})
#sample_weight[0][sample_weight[0]==0.3] = 1
sample_weight[0][sample_weight[0]==0.3] = 0.5
#XGB
params = {
...
dtrain = xgb.DMatrix(features,label = target_train, weight=sample_weight.values)
...
def giveResultOnTestset_XGBLR():
    ...
    #XGBOOST
    target_test = pd.DataFrame( gbm.predict(xgb.DMatrix(features_test), ntree_limit=gbm.best_iteration) )
    ...
    df_res.to_csv("v3_20_2 weighted sample.csv",header=None,index=False)
    return df_res
